...
namespace PBD_pendulum_simulation.src.fe_objects
{
    ...
    public class pendulum_values_store
    {
        ...
        public double screen_length = 0.0;
    ...
    public class pendulum_data_store
    {
        ...
        const double FIXED_DT = 1.0 / 12.0;  // 1.0 / 120.0;
        const int CONSTRAINT_ITERATIONS = 10;
        const double VELOCITY_DAMPING = 1.0;// 0.99999; // very mild
        ...
        public pendulum_data_store(List<double> masses, List<double> lengths, List<double> initial_angles_deg)
        {
            ...
            for (int i = 0; i < p_mass.Count; i++)
            {
                ...
                // Length scaling
                // -------------------------------
                double length_ratio = (p_mass[i].length_value / total_length) * TOTAL_SCREEN_LENGTH;


                Vector2 p1 = NextPoint(p0, p_mass[i].initial_angle_rad, length_ratio);
                ...
                // Add screen length
                p_mass[i].screen_length = Math.Sqrt((p1.X - p0.X) * (p1.X - p0.X) + (p1.Y - p0.Y) * (p1.Y - p0.Y));
        ...
        private Vector2 NextPoint(Vector2 origin, double angle, double length)
        {
        ...
        public void reset_simulation()
        {
            ...
            for (int i = 0; i < p_mass.Count; i++)
            {
                ...
                // Mass scaling
                // -------------------------------
                float screen_mass_size = (float)gvariables_static.GetRemap(maxMass, 0.0, MAX_MASS_SIZE, MIN_MASS_SIZE,
                    p_mass[i].mass_value);
                ...
                // Masses
                p_mass[i].fe_mass.update_pointmass(p1, screen_mass_size);
        ...
        private void StepPhysics(double dt)
        {
            ...
            for (int iter = 0; iter < CONSTRAINT_ITERATIONS; iter++)
            {
                ...
                for (int i = 0; i < p_mass.Count; i++)
                {
                    ...
                    // Correction value
                    double C = d - p_mass[i].screen_length;
                    double corr = C / (w0 + w1);
            ...
            for (int i = 0; i < p_mass.Count; i++)
            {
                ...
                // p_mass[i].Velo_X *= VELOCITY_DAMPING;
                // p_mass[i].Velo_Y *= VELOCITY_DAMPING;
            }
            ...
            for (int i = 0; i < p_mass.Count; i++)
            {
                ...
                Vector2 p1 = new Vector2((float)p_mass[i].Pos_X, (float)p_mass[i].Pos_Y);
