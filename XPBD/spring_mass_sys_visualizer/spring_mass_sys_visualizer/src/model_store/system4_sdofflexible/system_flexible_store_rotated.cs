...
{
    public class system_flexible_store_rotated
    {
        // Geometry data
        ...
        private sdof_flexiblecollisionSolver sdofflexiblecollisionSolver;
        ...
        public system_flexible_store_rotated(double total_simulation_time)
        {
        ...
        private void PerformSolve()
        {
            ...
            double mass_m2 = 0.002 * 1.620; // 2 KG

            double stiff_k1 = 0.018; // Stiffness k1 spring
            double stiff_k2 = 0.018; // 0.045; // Stiffness k2 spring
            ...
            sdofflexiblecollisionSolver = new sdof_flexiblecollisionSolver(mass_m1, mass_m2, stiff_k1, stiff_k2, dampratio_zeta, gravity_g);
            ...
            sdofflexiblecollisionSolver.solve_sdof_collision_with_flexible_boundary(total_simulation_time,
            ...
            int time_points = sdofflexiblecollisionSolver.SimulationResults.TimePoints.Count;

            for (int i = 0; i < time_points; i++)
            {

                (List<double> displacement_at_t, List<double> velocity_at_t, List<double> acceleration_at_t)
                    = sdofflexiblecollisionSolver.SimulationResults.GetStateListAtTimeIndex(i);
        ...
        public void paint_sdof_flexibleboundary_rotated(ref Shader modelShader)
        {
...
gvariables_static.geom_transparency * 0.8f);
        ...
        public void update_sdof_flexibleboundary_collision_rotated(double elapsedRealTime)
        {
            ...
            (List<double> Displacement, List<double> Velocity, List<double> Acceleration, double contact_force)
                = sdofflexiblecollisionSolver.getResult_at_timet(elapsedRealTime);
            ...
            pointmass.updateCirclePosition(0, (float)(default_ptmass_location[0] + node1_mapped_displacement * scale_value), 0.0f);
            pointmass.updateCirclePosition(1, (float)(default_ptmass_location[1] + 5.0f + node2_mapped_displacement * scale_value), 0.0f);

            pointmass.UpdateVertexBuffers();


            springs.updateSpringPosition(0, (float)(default_ptmass_location[0] + node1_mapped_displacement * scale_value), 0.0f, -45.0f, 0.0f);
            ...
            if (contact_force > 0)
            {
                ...
                springs.updateSpringPosition(1,
                    (float)(-default_ptmass_location[1] + 5.0f + node2_mapped_displacement * scale_value), 0.0f,
                    (float)(default_ptmass_location[1] + 5.0f + node2_mapped_displacement * scale_value), 0.0f);
            ...
            else
            {
                ...
                springs.updateSpringPosition(1,
                     (float)(default_ptmass_location[0] + 5.0f + node1_mapped_displacement * scale_value), 0.0f,
                   (float)(default_ptmass_location[1] + 5.0f + node2_mapped_displacement * scale_value), 0.0f);
            ...
            velocity_vectors.updateVectorPosition(0, (float)(default_ptmass_location[0] + node1_mapped_displacement * scale_value), 10.0f,
                    vector_scale_value * (float)node1_mapped_velocity, 0.0f);

            acceleration_vectors.updateVectorPosition(0, (float)(default_ptmass_location[0] + node1_mapped_displacement * scale_value), 20.0f,
                    vector_scale_value * (float)node1_mapped_acceleration, 0.0f);
            ...
            velocity_vectors.updateVectorPosition(1, (float)(default_ptmass_location[1] + 5.0f + node2_mapped_displacement * scale_value), 10.0f,
                    vector_scale_value * (float)node2_mapped_velocity, 0.0f);

            acceleration_vectors.updateVectorPosition(1, (float)(default_ptmass_location[1] + 5.0f + node2_mapped_displacement * scale_value), 20.0f,
                    vector_scale_value * (float)node2_mapped_acceleration, 0.0f);
